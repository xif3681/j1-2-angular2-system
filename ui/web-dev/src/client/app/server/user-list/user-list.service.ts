...
export class UserListService {
   ...
   * 获取列表信息
   */
  getUserList(start,limit): Promise<string[]> {
    const url = `/manager/user/list?start=${start}&limit=${limit}`;
    return this.http
      .get(url)
      .toPromise()
      .then(
        (res: Response) => {
          if(res.status == 200){
            ...
            this.promptEmitService.change.emit(res.json().code.toString()+'list');
          }
        }
      )
